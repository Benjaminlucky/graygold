...
function PropertySkeleton() {
  return (
    <div className="animate-pulse">
      {/* Hero Image */}
      <div className="aspect-video bg-primary-100 rounded-2xl mb-8" />
      ...
      <div className="flex gap-3 mb-6">
        <div className="h-10 bg-primary-100 rounded w-40" />
        <div className="h-10 bg-primary-100 rounded w-40" />
      </div>

      {/* Price */}
      <div className="h-12 bg-primary-100 rounded w-1/2 mb-8" />

      {/* Features Grid */}
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="h-28 bg-primary-100 rounded-xl" />
        ))}
      </div>
